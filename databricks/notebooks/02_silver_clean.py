...
).saveAsTable(f"{catalog}.silver.listings_clean")
if len(rejected) > 0:
    spark.createDataFrame(rejected).write.format("delta").mode("append").option(
        "mergeSchema", "true"
    ).saveAsTable(f"{catalog}.silver.listings_rejected")
